 ...
 * @param {module:net.Socket} socket
 */
function sendFile(absolutePath, data, socket) {
    let emptyEnd = () => {

    }
    if (!fs.existsSync(absolutePath)) {
        lock.releaseLock(data.relativePath);
    } else {
        let isFirst = true;
        if (fs.statSync(absolutePath).size) {
            const readStream = fs.createReadStream(absolutePath, {highWaterMark: CHUNK_SIZE});
            readStream.on('data', (chunk) => {
                console.log(magenta(`Send path: ${data.relativePath}, chunk length: ${chunk.length}`));
                let event = new DataFormat(SEND_FILE_CHUNK_EVENT_NAME, data.relativePath, true, chunk.toString('base64'), isFirst);
                sendEvent(socket, event);
                isFirst = false;
            });
            readStream.on('end', () => {
                let event = new DataFormat(SEND_FILE_CHUNK_EVENT_NAME, data.relativePath, true, '', false, true);
                sendEvent(socket, event);
                lock.releaseLock(data.relativePath);
            });
        } else {
            ...
            sendEvent(socket, event);
            event = new DataFormat(SEND_FILE_CHUNK_EVENT_NAME, data.relativePath, true, '', false, true);
            sendEvent(socket, event);
            lock.releaseLock(data.relativePath);
