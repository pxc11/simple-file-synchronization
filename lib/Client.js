...
class Client extends Queue {
    constructor() {
        ...
        this.client.connect(clientConfig.PORT, clientConfig.HOST, () => {
            ...
            this.watchFilesChange = new WatchFilesChange(clientConfig.WATCH_PATH, clientConfig.OPTIONS, clientConfig.WATCH_DEBOUNCE_INTERVAL);
        ...
        setInterval(() => {
            if (this.eventList.length > 0) {
                ...
                console.log('consuming:', this.consuming);
                            ...
                            try {
                                ...
                                if (d.status) {
                                    ...
                                    ws.on('close', async () => {
                                        ...
                                        console.log("event", JSON.stringify(e), ' use time', Date.now() - time);
                                        resolve();
                                    });
                                    ws.on('error', (err) => {
    ...
    handEventFromClient(e) {
        ...
        return new Promise(async (f, r) => {  // f 用于 resolve，r 用于 reject
            ...
            console.error('Error in handEventFromClient 2:', err,  JSON.stringify(e2));
            this.eventList.unshift(e2);
        });
